...
class CascadedTileUpscaler:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                ...
                "passes": ("INT", {"default": 2, "min": 1, "max": 2}),
                "denoise_2": ("FLOAT", {"default": 0.7, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "The amount of denoising applied, lower values will maintain the structure of the initial image allowing for image to image sampling."}),
                "strength_2": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01 }),
                "conditioning_strength": ("FLOAT", {"default": 0.4, "min": 0.0, "max": 1.0, "step":0.1, "round": 0.01}),
                "upscale_method": (upscale_methods, {"default": "lanczos", "tooltip": "The method used to upscale the image between passes."}),
    ...
    def upscale(self, config_pipe_enabled, model, vae, seed, steps, cfg, sampler_name, scheduler, positive, negative, image, passes, conditioning_strength, control_net, upscale_method, megapixels, tile_resolution, tile_overlap, denoise=0.5, strength=0.5, denoise_2=0.4, strength_2=0.4, config_pipe_in=None, desaturate=0.2):          
        ...
        for i in range(passes):
            ...
            if counter == 0 and passes > 1:
                current_megapixel = megapixels / 2
            else:
                current_megapixel = megapixels

            samples = scale_to_total_pixels(samples, upscale_method, current_megapixel, resolution_steps=1)
            ...
            for i in range(len(tiled_images[0])):
                ...
                if counter > 0:
                    denoise = denoise_2
                    strength = strength_2
                    zeroed_out_cond = zero_out(positive)
                    positive = addWeighted(zeroed_out_cond, positive, conditioning_strength)
...
def scale_to_total_pixels(image, upscale_method, megapixels, resolution_steps):
...
def rescale_tile(image, tile_resolution):
    mp = tile_resolution ** 2 / 1024 ** 2
    print(f"Rescaling tile to {tile_resolution}x{tile_resolution}, mp={mp}")
    return scale_to_total_pixels(image, "lanczos", mp, resolution_steps=1)
